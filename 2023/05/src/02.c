...
int main(int argc, char **argv) {
	...
	if (seeds.count % 2 != 0) {
		fprintf(stderr, "We need pairs of values for this to work.\n");
		return 1;
	}

	DynArray seeds_really = {0};

	for (size_t i = 0; i < seeds.count; i += 2) {
		long range_start = seeds_arr[i];
		long range_size = seeds_arr[i+1];
		for (long j = 0; j < range_size; j += 1) {
			long value = range_start + j;
			dyn_array_add(&seeds_really, value);
		}
	}

	long *seeds_really_arr = dyn_array_as_type(seeds_really, long);
	...
	for (size_t i = 0; i < seeds_really.count; i += 1) {
		long seed = seeds_really_arr[i];
		...
		//fprintf(stderr, "Seed %ld, soil %ld, fertilizer %ld, water %ld, light %ld, temperature %ld, humidity %ld, location %ld.\n", seed, soil, fertilizer, water, light, temperature, humidity, location);
	}
