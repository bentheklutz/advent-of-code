...
string TakeWord(u8 *data, u64 max_length) {
	s32 charsize = 0;
	int end_string = 0;
	while (charsize < max_length) {
		u8 current_char = *(data+charsize);
	...
	string word = {charsize};
	word.data = data;
	return word;
}

string TakeNumber(u8 *data, u64 max_length) {
	s32 charsize = 0;
	int end_string = 0;
	while (charsize < max_length) {
		u8 current_char = *(data+charsize);
...
string TakeAlphaNumeric(u8 *data, u64 max_length) {
	s32 charsize = 0;
	int end_string = 0;
	while (charsize < max_length) {
		u8 current_char = *(data+charsize);
...
u32 StringToUnsignedInteger(string input) {
	int number_bad_characters = 0;
	u8 non_numerical_characters[128] = {0};
	int length = input.length;

	u32 result = 0;
	for (int index = 0; index < length; ++index) {
		int power_offset = length - index - 1;
	...
	if (number_bad_characters) {
		printf("found %d non-numerical characters. unless they were leading this affected results\n", number_bad_characters);
	}
	return result;
}
