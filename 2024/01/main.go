...
Once again consider your left and right lists. What is their similarity score?
	...
	bytes_read, err := input_file.Read(contents)

	fmt.Printf("Read %d bytes of the file into our buffer to process.\n", bytes_read)
	lines := strings.Split(string(contents), "\n")
	lines = lines[:len(lines)-1]
	fmt.Printf("The file consists of %d lines\n", len(lines))

	lefts := make([]int, len(lines))
	rights := make([]int, len(lines))
	similarity_scores := make([]int, len(lines))
	for idx, line := range lines {
		...
		lefts[idx], _ = strconv.Atoi(nums[0])
		rights[idx], _ = strconv.Atoi(nums[1])
	}
	...
	for i := 0; i < len(lines); i++ {
		diff := math.Abs(float64(lefts[i] - rights[i]))
		total_diff += diff
	}
